...
GETCMD_LOOP:
    ...
    ;---------------------------------------
    ; Replace space with EOL in cmdline buffer
    ; TODO command length could be stored in array at bottom
    ;---------------------------------------
GETCMD_REPL:
        LDA     #EOL
        STA     (INBUFF),Y
        ...
        CMP     #' '        ; If space then go to next char
        BEQ     GETCMD_REPL
        TYA
        STA     CMDSEP,X
        INX
...
BREADY  .BYTE       '#FUJINET NOS v0.1 (alpha)',EOL
